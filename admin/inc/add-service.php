...
if ($usr->getSession() && $usr->isStaff($usr->getSession())) {
  ...
  if (isset($_POST['saleid']) && isset($_POST['customerName'])  && isset($_POST['customerAddress'])  && isset($_POST['customerPhone'])  && isset($_POST['serviceDate']) && isset($_POST['returndate']) && isset($_POST['serviceCharges']) ) {
  	...
  	$getReceipt = $pos->getSaleBySID($_POST['saleid']);
  	if ($getReceipt) {
        $data = array();
        $data['customerName'] = $_POST['customerName'];
        $data['customerAddress'] = $_POST['customerAddress'];
        $data['customerPhone'] = $_POST['customerPhone'];
        $adCust = $pos->insertCustomer($data);

  			if (!$adCust['error']) {
            $sData = array();
            $sData['saleid'] = $_POST['saleid'];
            $sData['customerId'] = $adCust['msg'];
            $sData['serviceDate'] = $_POST['serviceDate'];
            $sData['returndate'] = $_POST['returndate'];
            $sData['serviceCharges'] = $_POST['serviceCharges'];
            $serin = $pos->insertService($sData);
            if (!$serin['error']) {
            ...
            $output .= '<div class="col-sm-12 col-lg-7">'.$_POST["customerName"].'</div>';
            $output .= "</div>"; 

            $output .= '<div class="row">';
            $output .= '<div class="col-sm-12 col-lg-3">ADDRESS</div>';
            $output .= '<div class="col-sm-12 col-lg-7">'.$_POST["customerAddress"].'</div>';
            $output .= "</div>"; 

            $output .= '<div class="row">';
            $output .= '<div class="col-sm-12 col-lg-3">PHONE</div>';
            $output .= '<div class="col-sm-12 col-lg-7">'.$_POST["customerPhone"].'</div>';
            $output .= "</div>"; 
            ...
            $output .= '<div class="col-sm-12 col-lg-7">'.$formats->formatTimeStamp('m/d/Y h:m A',$_POST["serviceDate"]).'</div>';
            $output .= "</div></div>"; 

            $output .= '<div class="row">';
            $output .= '<div class="col-sm-12 col-lg-3">RETURN</div>';
            $output .= '<div class="col-sm-12 col-lg-7">'.$formats->formatTimeStamp('m/d/Y h:m A',$_POST["returndate"]).'</div>';
            $output .= "</div></div>"; 

            $output .= '<div class="row">';
            $output .= '<div class="col-sm-12 col-lg-3">CHARGE (BDT)</div>';
            $output .= '<div class="col-sm-12 col-lg-7">'.$_POST["serviceCharges"].'</div>';
            $output .= "</div></div>"; 


            $output .= '<br>=========================================<br><p>WE ARE GLAD TO SERVICE YOU !!!</p><br>FOR FUTURE REFERENCE KINDLY PROVIDE THE RECEIPT</div>';
            $response['error'] = false;
            $response['msg'] = $output;
            }
            else{
              $response['error'] = true;
              $response['msg'] = "Error while inserting service record";
            }
        }
        else{
          $response['error'] = true;
          $response['msg'] = $adCust['msg'];
        }
  	}
  	else{
  		$response['error'] = true;
